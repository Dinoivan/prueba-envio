...
@Component
public class ContactoAprobadaRechazadaODNotificacion extends NotificarMail {
    ...
    public String enviar(OrdenDespacho ordenDespacho, Usuario proveedor, Usuario comprador){
        ...
        if (proveedor == null && comprador != null) { // email a comprador (cliente)
            ...
            _vel_NombreDestinatario = StrUtils.escapeComma(ordenDespacho.getProveedorRazonSocial());
            _vel_TextoEmpresa = "de la empresa " + sociedad.getRazonSocial();
        }
        else {
            String mensaje = "Error en los datos de proveedor/comprador para el envio de correo de Publicacion de OC: " + ordenDespacho.getNumeroOrdenCompra();
            logger.error(mensaje);
            return mensaje;
        }

        if(OrdenCompraEstadoEnum.RECHAZADA.getId() == ordenDespacho.getIdEstadoOrdenCompra().intValue()){
            _vel_mensajeRechazo = String.format(MENSAJE_RECHAZO, StrUtils.escapeComma(ordenDespacho.getMotivoRechazo()));
        ...
        try{
            ...
            logger.error(emailContacto + " - " + _vel_accion + " - " + _vel_NroOrdenCompra);
            logTransaccion.setEnvioTrama(emailContacto + " - " + _vel_accion + " - " + _vel_NroOrdenCompra);
        ...
        logger.error("<--KG_LOG-->:LOG_CORREO_10");
